      ...
      ?>
        ...
        <tr>
          ...
          <td class="nowrap" style="text-align: right;">
            <?php echo number_format($ligne->montantv, 2, '.', ' '); ?>
          ...
          <td>
            <?php echo strlen($ligne->remarquebon) > 50 ? substr($ligne->remarquebon, 0, 50) . "..." : $ligne->remarquebon; ?>
          </td>
...
if ($_POST['act'] == 'rech') {
  ...
  ?>
    ...
    <tr>
      ...
      <td>
        <?php echo $ligne->poid * $ligne->qte_vendu; ?> g
      </td>
  ...
  if ($_POST['bon_commande'] != 0) {
    ...
    foreach ($cmd_details as $cmd_detail) {
      ...
      $query = "INSERT INTO detail_vente(id_produit, qte_vendu , qte_restante , prix_produit, remise, valunit, unit, id_vente) VALUES ($id_produit, $qte_vendu,$qte_vendu, $prix_produit, $remise, $valunit, 'unit', $dernier_vente)";
...
where a.id_vente=$dernier_vente group by  da.id_produit");
  ...
  try {
    ...
    connexion::getConnexion()->exec("DELETE FROM vente WHERE id_vente = " . $_POST["id"]);
  ...
  // if ($augmentatation && $augmentatation != 0) {

  //   $prix_v = $prod->prix_achat_i + (($prod->prix_achat_i * $augmentatation) / 100);
  // } else {
  //   $cl_prix_vente = connexion::getConnexion()->query("SELECT prix_vente FROM client WHERE id_client = " . $_POST['id_client'])->fetch(PDO::FETCH_COLUMN);

  //   switch ($cl_prix_vente) {
  //     case 2:
  //       $prix_v = $prod->prix_vente2;
  //       break;
  //     case 3:
  //       $prix_v = $prod->prix_vente3;
  //       break;
  //     default:
  //       $prix_v = $prod->prix_vente;
  //       break;
  //   }
  // }
  ...
  if ($augmentatation && $augmentatation != 0) {
    ...
    $prix_v = $prod->prix_achat + (($prod->prix_achat * $augmentatation) / 100);
  } else {
    $cl_prix_vente = connexion::getConnexion()->query("SELECT prix_vente FROM client WHERE id_client = " . $_POST['id_client'])->fetch(PDO::FETCH_COLUMN);
