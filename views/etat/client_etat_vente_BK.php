...
$data = connexion::getConnexion()->query("select v.remarquebon, v.id_vente,v.date_vente, sum(dv.prix_produit*(1-dv.remise/100)*dv.qte_vendu)as
...
and  (v.date_vente  between '" . $_POST["dd"] . "' and '" . $_POST["df"] . "') group by  v.id_vente  order by v.date_vente ASC");
?>
  ...
  <tr class="row">
   ...
   <?php foreach($data as $ligne) { ?>
          ...
          $data2 = connexion::getConnexion()->query("SELECT v.numbon, dv.id_detail,p.designation,p.poid,p.designation_ar,dv.prix_produit,dv.qte_vendu  
          FROM  detail_vente dv 
          left join vente v ON v.id_vente = dv.id_vente
          left join produit p on (p.id_produit=dv.id_produit) 
          where dv.prix_produit > 0 AND dv.id_vente=" . $ligne['id_vente'] . " order by dv.id_detail desc");
             ...
             ?>
              <tr>
                 <td><?php echo $ligne["date_vente"] ?></td>
                  ...
                  <td scope="col" style="text-align:right"><?php echo $ligne2["qte_vendu"];
                  ...
                  <td style="text-align:right;padding-right:15px"
                      scope="col"><?php 
			         $sum_ventes += $ligne2["qte_vendu"] * $ligne2["prix_produit"]*(1-$ligne2["remise"]/100);
			         echo number_format($ligne2["qte_vendu"] * $ligne2["prix_produit"]*(1-$ligne2["remise"]/100), 2, '.', ' ');
                     ?></td>
                  <td><?php echo $ligne["remarquebon"] ?></td>
              </tr>
          <?php }
          //} ?>
   <?php } ?>
   ...
   <?php
	 ...
	 $query = "select SUM(dv.prix_produit * dv.qte_vendu * (1 - dv.remise / 100)) AS Total from vente v left join detail_vente dv ON dv.id_vente = v.id_vente left join produit p ON p.id_produit = dv.id_produit where v.numbon>0 and v.id_client=" . $_GET["id"] . " AND v.date_vente < '" . $_POST["dd"] . "'";
