...
<h3 align="center" id="entetee1"  > Etat des Reglements des ventes du <?php echo dateFormat( $_POST['dd']); ?> a <?php echo dateFormat($_POST['df']); ?>
</h3>
...
$total_montant=0;$i=0;
foreach($data_eleveurs as $rep_eleveurs ){$i+=1;
$result=connexion::getConnexion()->query("SELECT v.id_vente,r.montant,r.etat,r.id_reg,date_reg,mode_reg,r.remarque  ,num_cheque FROM `reg_vente` r inner join vente v on v. id_vente=r.id_vente  WHERE  v.id_client= ".$rep_eleveurs->id_client."  and   (date_reg between '".$_POST['dd']."' and '".$_POST['df']."') order by v.id_vente asc");
$data=$result->fetchAll(PDO::FETCH_OBJ);
 ?>
 <fieldset id="<?php echo "A".$i;?>"><legend> <strong>
  <?php echo $rep_eleveurs->nom." ".$rep_eleveurs->prenom;  ?> 
  <?php design_ar($rep_eleveurs->nom." ".$rep_eleveurs->prenom,$rep_eleveurs->nom_prenom_ar);?>...</strong>
...
if($rep->montant>0){
 ?>
   <tr <?php if( $rep->etat==1) echo 'style="background:#D9FCCE"'; ?>>
        <th width="20%" ><?php echo $rep->id_vente;?></th>
...
<table class="datatables"  border=1  > 
    <tr class="row">
    <th  width="40%" colspan="4">Total G&eacute;n&eacute;rale </th> 
