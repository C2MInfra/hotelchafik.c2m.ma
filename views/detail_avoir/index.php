...
$data = $detail_avoir->selectAllValide($id);
?>
...
<div class="container-fluid disable-text-selection">
 ...
 <input type="hidden" id="id_avoir" value="<?php echo $id  ?>"/>
        ...
        <div class="float-sm-right text-zero">
          <button type="button" class="btn btn-primary btn-lg  mr-1 url notlink" data-url="detail_avoir/add.php?id=<?php echo $id ?>" >AJOUTER</button>
        </div>
              ...
              <table class="table  responsive table-striped table-bordered table-hover" id="datatables" >
                ...
                <?php
                  ...
                  <td>
                    ...
                    value="<?php echo $ligne->qte_rendu; ?>"/>
                    <span
                      id="qte<?php echo $ligne->id_detail; ?>">
                    </span>
                  </td>
                  <td style="text-align:right;" width="90">
                    <?php echo number_format($ligne->qte_rendu * $prixr, 2, '.', '');
                    $total += $ligne->qte_rendu * $prixr;
                    ?>
                  </td>
                  <td>
                  	<?php
       ...
       $("input.datepicker").datepicker({
           ...
           $('#datatables').dataTable({
                ...
                dom: 'Bfrtip',
                  buttons: [
                      {
                          extend: 'excelHtml5',
                          title:"liste des detail_avoirs N° <?php echo $id ?>",
                          exportOptions: {
                               columns: [ 0, 1, 2,3,4 ]
                          }
                      },
            ...
            $('body').on( "click",".delete", function( event ) {
             event.preventDefault();
                ...
                swal({
                 title: 'Êtes-vous sûr?',
                  text: "Voulez vous vraiment Supprimer ce detail_avoir !",
                  type: 'warning',
                ...
                success: function (data) {
                   ...
                   swal(
                      'Supprimer',
                      'detail_avoir a ete bien Supprimer',
                      'success'
                    ).then((result) => {
        ...
        $('body').on( "click",".archive", function( event ) {
             event.preventDefault();
                ...
                swal({
                 title: 'Êtes-vous sûr?',
                  text: "Voulez vous vraiment Archiver ce detail_avoir !!",
                  type: 'warning',
                  showCancelButton: true,
                  ...
                  var btn = $(this); 
                    ...
                    success: function (datas) {
                        var data = datas.split(';;;');
                                $('#exampleModalRight .modal-title').html("Etat detail_avoir "+data[1]);
                                $('#idstatic').val(data[0]);
                                }
                            });
...
var value = $(this).data('id');
    ...
    alert("id_detail=" + value + "&id_avoir=" + $('#id_avoir').val() + "&remise=" + remise + "&prix_produit=" + prix + "&qte_rendu=" + qte + "&id_produit=" + $('#' + value).children("td:eq(1)").attr('id') + "&qte_renduh=" + $('#' + value).find("label:eq(2)").html())
    $.ajax({
    type: "POST",
    url: "<?php echo BASE_URL.'views/detail_avoir/' ;?>controle.php",
    data: "act=update_detail&id_detail=" + value + "&id_avoir=" + $('#id_avoir').val() + "&remise=" + remise + "&prix_produit=" + prix + "&qte_rendu=" + qte + "&id_produit=" + $('#' + value).children("td:eq(1)").attr('id') + "&qte_renduh=" + $('#' + value).find("label:eq(2)").html(),
    success: function (data) {
    ...
    $('#' + value).children("td:eq(6)").html(parseFloat(prix) *  parseFloat(qte) * (1 - remise/100) );
    $('#qte' + value).html("");
