...
if($_POST['act']=='getcat' && isset($_POST['id_categorie'])) {
   $id = $_POST['id_categorie'];
		...
		$data = $fournisseur->selectChamps("code_bar","id_categorie =  $id",'',"id_fournisseur","desc","1");
     die ($data[0]->code_bar);
}
...
elseif ($_POST['act']=='archive') {
	...
	$fournisseur=new fournisseur(); 
	$fournisseur->archiver($_POST["id"],$_POST["val"], auth::user()["id"]);
...
elseif ($_POST['act']=='getName') {
	...
	$fournisseur=new fournisseur(); 
	$oldvalue=$fournisseur->selectById($_POST['id']);
...
$fournisseur=new fournisseur();
$res=$fournisseur->selectById($_POST["id"]);
	$total=0;
$poids = 0;
$qte = 0;
			if($_POST['etatfournisseur']=="achat"){
			$result=connexion::getConnexion()->query("select concat_ws(' ',f.raison_sociale)as fournisseur ,
a.date_achat,da.prix_fournisseur,sum(da.qte_achete) as qte_achete ,da.prix_fournisseur*da.qte_achete as total
from fournisseur f left join  achat a on a.id_fournisseur=f.id_fournisseur left join detail_achat da on da.id_achat=a.id_achat where (a.date_achat between '". $_POST['dd'] ."' and '". $_POST['df'] ."')  and da.id_fournisseur=". $_POST["id"] ." group by f.id_fournisseur");
			$data = $result->fetchAll(PDO::FETCH_OBJ);
			?>

			<center>
			<p><h3> Liste des achats de fournisseur :</span>
			...
			<td> <?php echo number_format($ligne->prix_fournisseur,2,"."," "); ?> &nbsp;&nbsp;</td>
			...
			v.date_vente,dv.prix_fournisseur,sum(dv.qte_vendu) as qte_vendu ,dv.prix_fournisseur*dv.qte_vendu as total
			from fournisseur c left join  vente v on v.id_fournisseur=c.id_fournisseur left join detail_vente dv on dv.id_vente=v.id_vente where (v.date_vente between '". $_POST['dd'] ."' and '". $_POST['df'] ."')  and dv.id_fournisseur=". $_POST["id"] ." group by c.id_fournisseur");
			$data = $result->fetchAll(PDO::FETCH_OBJ);
		
			?>
			<center>
			<p><h3>Liste des Vente de fournisseur :</span>
			<?php echo $res["designation"] ." ( ".$res["poid"] ." ) " ?>  <br>
			<b dir="rtl">
